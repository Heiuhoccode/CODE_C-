...
int duongcheo_cot(int a[5][5],int S){
	...
	for(int i=0; i<5; i++) {COT4 += a[i][3]*pow(10,5-i-1);COT5 += a[i][4]*pow(10,5-i-1);cot4 += a[i][3];cot5 += a[i][4];}
...
int main(){
	...
	for(int i=10001; i<=99999; i++) {
		if(snt(i)==1 && tong(i) == S) {int k=4, a = i;
			while(a>0) {int h = a%10;b[index][k] = h;k--;a/=10;}
			index++;}}
	
	// vong lap dien cac so vao o thu 1, 2, 3, 4, 5.
	int soluong = 0, x0 = 0;
	while(x0<index){
	if(b[x0][0]!=0&&b[x0][1]!=0&&b[x0][2]!=0&&b[x0][3]!=0&&b[x0][4]!=0){for(int i=0; i<5; i++) {a[0][i]=b[x0][i];}
	// vong lap dien cac so vao o thu 6, 7, 8, 9.
	int x2=0;
	while(x2<index){if(a[0][0]==b[x2][0] && b[x2][0]!=0 && b[x2][1]!=0 && b[x2][2]!=0 && b[x2][3]!=0 && b[x2][4]!=0){
	for(int i=0; i<5; i++){a[i][0] = b[x2][i];}
	// vong lap dien cac so vao o thu 10, 11, 12.
	int x3=0;
	while(x3<index){if(a[0][4]==b[x3][0]&&a[4][0]==b[x3][4]){for(int i=0; i<5; i++){a[i][5-1-i]=b[x3][i];}
	// vong lap dien cac so vao o thu 13, 14, 15.
	int x4=0;
	while(x4<index){if(a[1][0]==b[x4][0]&&a[1][3]==b[x4][3]){for(int i=0; i<5; i++) {a[1][i]=b[x4][i];}
	// vong lap dien cac so vao o thu 16, 17.
	int x5=0;
	while(x5<index){if(a[0][1]==b[x5][0]&&a[1][1]==b[x5][1]&&a[3][1]==b[x5][3]){
	for(int i=0; i<5; i++){a[i][1]=b[x5][i];}
	// vong lap dien cac so vao o thu 18, 19.
	int x6=0;
	while(x6<index){if(a[2][0]==b[x6][0]&&a[2][1]==b[x6][1]&&a[2][2]==b[x6][2]){
	for(int i=0; i<5; i++) {a[2][i]=b[x6][i];}
	// vong lap dien cac so vao o thu 20, 21.
	int x7=0;
	while(x7<index){if(a[0][2]==b[x7][0]&& a[1][2]==b[x7][1]&& a[2][2]==b[x7][2]) {
	for(int i=0; i<5; i++) {a[i][2]=b[x7][i];}
	// vong lap dien cac so vao o thu 22, 23.
	int x8=0;
	while(x8<index) {if(a[3][0]==b[x8][0]&&a[3][1]==b[x8][1]&&a[3][2]==b[x8][2]) {
	for(int i=0; i<5; i++) {a[3][i] = b[x8][i];}
	// vong lap dien cac so vao o thu 24, 25.
	int x9=0;
	while(x9<index) {if(a[4][0]==b[x9][0]&&a[4][1]==b[x9][1]&&a[4][2]==b[x9][2]) {
			for(int i=0; i<5; i++) {a[4][i]=b[x9][i];}
			// Kiem tra duong cheo chinh va hai cot 4,5.
			if(duongcheo_cot(a,S)==1){soluong++;break;} 
	else x9++;}else x9++;}x8++;}else x8++;}x7++;}else x7++;}x6++;}else x6++;}x5++;}
	else x5++;}x4++;}else x4++;}x3++;}else x3++;}x2++;}else x2++;}x0++;}else x0++;}
	
	cout <<  "Tong tat ca cac truong hop thoa man la: "<< soluong << endl;
	int cnt;
	if(S==17){cnt=5;cout << "5 cau hinh dau tien la:\n";}
	else {cnt=1e9;cout << "Tat ca cac cau hinh la:\n";}
	
	int x1 = 0;
	while(x1<index&&cnt>0){
	if(b[x1][0]!=0&&b[x1][1]!=0&&b[x1][2]!=0&&b[x1][3]!=0&&b[x1][4]!=0){
	for(int i=0;i<5;i++){a[0][i]=b[x1][i];}	
	// vong lap dien cac so vao o thu 6, 7, 8, 9.
	int x2=0;
	while(x2<index&&cnt>0){
	if(a[0][0]==b[x2][0]&&b[x2][0]!=0&&b[x2][1]!=0&&b[x2][2]!=0&&b[x2][3]!=0&&b[x2][4]!=0){
	for(int i=0;i<5;i++){a[i][0]=b[x2][i];}
	// vong lap dien cac so vao o thu 10, 11, 12.
	int x3=0;
	while(x3<index&&cnt>0){
	if(a[0][4]==b[x3][0]&&a[4][0]==b[x3][4]){
	for(int i=0;i<5;i++){a[i][5-1-i]=b[x3][i];}
	// vong lap dien cac so vao o thu 13, 14, 15.
	int x4=0;
	while(x4<index&&cnt>0){
	if(a[1][0]==b[x4][0]&&a[1][3]==b[x4][3]){
	for(int i=0;i<5;i++){a[1][i]=b[x4][i];}
	// vong lap dien cac so vao o thu 16, 17.
	int x5=0;
	while(x5<index&&cnt>0){
	if(a[0][1]==b[x5][0]&&a[1][1]==b[x5][1]&&a[3][1]==b[x5][3]){
	for(int i=0;i<5;i++){a[i][1]=b[x5][i];}
	// vong lap dien cac so vao o thu 18, 19.
	int x6=0;
	while(x6<index&&cnt>0){
	if(a[2][0]==b[x6][0]&&a[2][1]==b[x6][1]&&a[2][2]==b[x6][2]) {
	for(int i=0;i<5;i++){a[2][i]=b[x6][i];}
	// vong lap dien cac so vao o thu 20, 21.
	int x7=0;
	while(x7<index&&cnt>0){
	if(a[0][2]==b[x7][0]&& a[1][2]==b[x7][1]&& a[2][2]==b[x7][2]) {
	for(int i=0;i<5;i++){a[i][2]=b[x7][i];}
	// vong lap dien cac so vao o thu 22, 23.
	int x8=0;
	while(x8<index&&cnt>0){
	if(a[3][0]==b[x8][0]&&a[3][1]==b[x8][1]&&a[3][2]==b[x8][2]){
	for(int i=0;i<5;i++){a[3][i]=b[x8][i];}
	// vong lap dien cac so vao o thu 24, 25.
	int x9=0;
	while(x9<index&&cnt>0){
	if(a[4][0]==b[x9][0]&&a[4][1]==b[x9][1]&&a[4][2]==b[x9][2]){
	for(int i=0;i<5;i++){a[4][i]=b[x9][i];}
	// Kiem tra duong cheo chinh va hai cot 4,5.
	if(duongcheo_cot(a,S)==1){cnt--;
		for(int i=0;i<5;i++){for(int j=0;j<5;j++){cout<<a[i][j]<<" ";}
