...
class HoaDon{
    ...
    long long soluong, thanhtien, loinhuan;
    public:
    friend istream &operator >> (istream &in, HoaDon &dshd){
        ...
        dshd.thanhtien = giaban[dshd.mamh]*dshd.soluong;
        dshd.loinhuan = (giaban[dshd.mamh] - giamua[dshd.mamh]) * dshd.soluong;
        return in;
    }
    friend bool cmp(HoaDon,HoaDon);
    friend void sapxep(HoaDon,int);
    friend ostream &operator << (ostream &out, HoaDon &dshd){
        out << dshd.mhd <<" ";
        out << ten[dshd.makh]<<" ";
        out << diachi[dshd.makh]<<" ";
        out << tenhang[dshd.mamh]<<" ";
        out << dshd.soluong << " "<< dshd.thanhtien <<" ";
        out << dshd.loinhuan<<endl;
        return out;
    }
};

bool cmp(HoaDon a, HoaDon b){
    if(a.loinhuan == b.loinhuan) return true;
